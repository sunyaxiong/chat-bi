    ...
    def query_db(db_info:dict, fmt_sql:str, user_id:str, trace_id:str):
        ...
        try:
            if source_type == 'spark':
                # 使用Spark连接器
                from ..db.spark import get_spark_conn, fetch
                conn = get_spark_conn(db_info['host'], db_info['port'], db_info['user'], db_info['pwd'], db_info['db'])
                rows, row_count = fetch(fmt_sql, conn)
            else:
                # 使用MySQL连接器（默认）
