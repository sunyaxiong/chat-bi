...
def template_command(args):
    ...
    tables = args.tables
    datasource = getattr(args, 'datasource', 'mysql')
    print(f"场景: {scenario}, 数据源: {datasource}")
    print(f"表列表: {tables}")
    ...
    # 根据数据源类型获取连接
    if datasource == 'spark':
        from server.db import spark
        db_infos = conf.get_mysql_conf_by_question(scenario)
        db_info = db_infos[0]
        conn = spark.get_conn(db_info['host'], db_info['db'])
    else:
        db_infos = conf.get_mysql_conf_by_question(scenario)
        db_info = db_infos[0]
        conn = mysql.get_conn(
...
def main():
    ...
    parser_template.add_argument(
        "--datasource", type=str, default="mysql", choices=["mysql", "spark"], help="数据源类型")
